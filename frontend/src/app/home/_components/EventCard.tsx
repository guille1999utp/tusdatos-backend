...
export default function Cards() {
  ...
  return (
    <div
      ref={containerRef}
      className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 max-w-[90rem] mx-auto gap-4 p-4"
    >
      {cards.map((card, index) => {
        return (
          <div
            className="card-container relative aspect-square rounded-2xl overflow-hidden cursor-pointer"
            id={card.id}
            key={card.id}
          >
            ...
            <div className="card-title absolute bottom-8 left-8">
              <h3
                style={{ color: COPY_COLOR }}
                className="text-[clamp(1.5rem,2.5vw,2.5rem)] font-[450] leading-tight tracking-tight"
              >
                {card.title}
              </h3>
            </div>
