...
export const PinContainer = ({
  ...
  /** Si es false, no se renderiza enlace (evita navegación y abrir pestañas). */
  interactive?: boolean;
}) => {
  ...
  const inner = (
    <>
      ...
      <PinPerspective title={title} href={href} interactive={interactive} />
    </>
  );

  if (!interactive) {
    return (
      <div className={shellClass} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        {inner}
      </div>
    );
  }
...
export const PinPerspective = ({
  ...
  const titleChip = interactive && href ? (
    ...
    >
      <span className="relative z-20 text-xs font-bold inline-block py-0.5">{title}</span>
      <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-emerald-400/0 via-emerald-400/90 to-emerald-400/0 transition-opacity duration-500 group-hover/btn:opacity-40"></span>
    </a>
  ) : (
    <span className="relative flex space-x-2 items-center z-10 rounded-full py-0.5 px-4 ring-1 ring-white/10 ">
      <span className="relative z-20 text-xs font-bold inline-block py-0.5">{title}</span>
    </span>
  );
